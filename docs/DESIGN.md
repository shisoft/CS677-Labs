...
To ensure cache consistency, catalog coordinator will send invalidate messages to frontend server, to ensure end users can see most recent results. 
...
Because this assignment requires a working cluster that can be deployed on multiple physical machines, dockerize will be helpful when debugging a distributed system, in combination with some orchestration tools like docker compose and Kubernetes. 

This project have 2 docker containers for frontend and backend each. Their files lying under `src/pygmy-frontend` and `src/pygmy-backend`, `Dockerfile` tells how containers should be built. We would like to enabling build containers from our Rust source code we wrote. So, operators can build the container for different platforms and hardware. Each time when we made some modification on our code, we can build the container directly and then use `docker-compose` command to put it in the test with distributed setup.

Docker build in our build process are seprated in 2 stages. First state prepares the binary build envoritement and then build the execution file. For backend servers, this stage will install SQLite header files and set Rust compiler to nightly. After the code build envorinment is set, it will invoke rust compile to compile the binary. 

In the second stage, the binary files from the first stage will be copied to the file system of the second stage, and then setup runtime envorinment for the binary. For backend server, it will install SQLite dynamic library.

The reason for us to use multi-state build is to reduce the size of our container as much as possible, because we don't need to ship Rust toolchains with the container, just the binary it build. The toolchain for Rust itself can cost about 450MB, when the binary we build only costs below 50MB and they can be functional in the same way. 

We have a `docker-compose.yml` in `src/` directory. This is used for our test envorinment without transferring the container to remote machine but still main its distributed attributes. This setup will start 3 order servers, 3 catalog servers, and one fontend server. Configuration for each of these 7 nodes will be done by setting envorinment variables for each container. Frontend ports are exposed by the host, nodes can find each other by their container name and predefined ports.
