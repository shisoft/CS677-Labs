...
echo "CAT_SERVER_ADDR = '$CATALOG_SERVER'" >> .env
echo "ORDER_SERVER_ADDR = '$ORDER_SERVER'" >> .env
...
ssh $DEPLOY_USER@$CATALOG_SERVER "/bin/sh -c 'cd $REMOTE_DIR && chmod +x ./catalog-server && ./catalog-server > catalog-server.log &'"

echo "Running order server on $ORDER_SERVER"
ssh $DEPLOY_USER@$ORDER_SERVER "/bin/sh -c 'cd $REMOTE_DIR && chmod +x ./order-server && ./order-server > order-server.log &'"